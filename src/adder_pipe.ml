...
module Implementation(Comb : Comb.S) = struct
  ...
  let rec create_stage_impl ~carry_ins ~input_parts =
    ...
    | (a :: b :: tl_parts), (carry_in :: tl_carry_in) ->
      let this_sum =
        let w = width a in
        uresize a (w + 1) +: uresize b (w + 1) +: uresize carry_in (w + 1)
      in
  ...
  let create_stage
    ...
    let { partial_result; carries } = create_stage_impl ~carry_ins ~input_parts in
...
module With_interface(M : sig
  ...
  let create ~stages (_ : Scope.t) ({ I. clock; enable; data; valid } : _ I.t) =
    ...
    let { result; carries = _ } = create ~stages ~pipe data in
