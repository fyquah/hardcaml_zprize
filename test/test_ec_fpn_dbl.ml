...
module Ec_fpn_dbl = Ec_fpn_dbl.With_interface(struct let bits = 377 end)
...
let modulo_inverse x =
  ...
  let ret =
    if Z.lt ret Z.zero then (
      Z.(ret + p)
    ) else (
      ret
    )
  in
  if not (Z.gt ret (Z.of_int (-1)) && Z.lt ret p) then (
    raise_s [%message (ret : Utils.z) (p : Utils.z)];
  );
  ret
;;
...
let compute_expected jacobian =
  Ark_bls12_377_g1.mul (jacobian_to_affine jacobian) ~by:2
;;
...
let test ?(debug = false) ~(config : Config.t) ~(sim : Sim.t) ~montgomery test_inputs =
  ...
  let cycle () =
    ...
    if Bits.is_vdd !(outputs.valid_out) then (
      let to_z r =
        let r = Bits.to_z ~signedness:Unsigned r in
        if montgomery then
          transform_from_montgomery r
        else
          r
      in
  ...
  List.iter test_inputs ~f:(fun { Jacobian. x; y; z } ->
      let of_z a =
        Bits.of_z ~width:377
          (if montgomery then transform_to_montgomery a else a)
      in
  ...
  if len_test_inputs <> len_test_outputs then (
    raise_s [%message
  ...
  List.map2_exn test_inputs test_outputs ~f:(fun test_input obtained ->
      ...
      if [%equal: Ark_bls12_377_g1.affine] obtained expected then (
        Ok ()
      ) else (
