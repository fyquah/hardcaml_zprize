...
module Subtracter64 = Subtracter_pipe.With_interface(struct
    let bits = 64
  end)
module Sim = Cyclesim.With_interface(Subtracter64.I)(Subtracter64.O)

let create_sim ~stages =
  let scope = Scope.create ~flatten_design:true () in
  Sim.create (Subtracter64.create ~stages scope)
;;
...
let%expect_test "" =
  ...
  List.iter test_cases ~f:(fun { x; y; p } ->
      inputs.p := Bits.of_z ~width:Subtracter64.bits p;
      inputs.x := Bits.of_z ~width:Subtracter64.bits x;
      inputs.y := Bits.of_z ~width:Subtracter64.bits y;
      inputs.valid <--. 1;
      cycle ();
    );
  inputs.valid <--. 0;
