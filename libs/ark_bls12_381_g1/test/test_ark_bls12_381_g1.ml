...
let sexp_of_z = Ark_bls12_381_g1.For_testing.sexp_of_z

let print_is_on_curve a =
  if Ark_bls12_381_g1.is_on_curve a
  then Stdio.printf "point is on curve!\n"
  else Stdio.printf "point is not on curve!\n"
;;

let%expect_test "Test accessors" =
  let a = Ark_bls12_381_g1.create ~x:(Z.of_int 2) ~y:(Z.of_int 3) ~infinity:true in
  Stdio.print_s ([%sexp_of: Ark_bls12_381_g1.affine] a);
  [%expect {| ((x 0x2) (y 0x3) (infinity true)) |}]
;;

let%expect_test "Print model coefficient and constants." =
  let a = Ark_bls12_381_g1.coeff_a () in
  let b = Ark_bls12_381_g1.coeff_b () in
  let modulus = Ark_bls12_381_g1.modulus () in
...
let%expect_test "Check points that should be on the curve" =
  let create = Ark_bls12_381_g1.create in
  ...
  |> List.iter ~f:(fun a -> assert (Ark_bls12_381_g1.is_on_curve a))
;;

let%expect_test "Display subgroup generator" =
  let a = Ark_bls12_381_g1.subgroup_generator () in
  let x = Ark_bls12_381_g1.x a in
  let y = Ark_bls12_381_g1.y a in
...
let%expect_test "Verify equality" =
  [ Ark_bls12_381_g1.subgroup_generator (), Ark_bls12_381_g1.subgroup_generator () ]
  |> List.iter ~f:(fun (a, b) -> assert ([%equal: Ark_bls12_381_g1.affine] a b))
;;

let%expect_test "Verify point-addition and point-multiplication give similar results." =
  let generator = Ark_bls12_381_g1.subgroup_generator () in
  for by = 1 to 10 do
    let from_mul = Ark_bls12_381_g1.mul generator ~by in
    let from_addition =
      Fn.apply_n_times ~n:(by - 1) (Ark_bls12_381_g1.add generator) generator
    in
    assert ([%equal: Ark_bls12_381_g1.affine] from_mul from_addition)
  done
;;
...
let%expect_test "Multiplying a point by zero" =
  let generator = Ark_bls12_381_g1.subgroup_generator () in
  print_s ([%sexp_of: Ark_bls12_381_g1.affine] (Ark_bls12_381_g1.mul ~by:0 generator));
...
let%expect_test "Multiply by an arbitrary [Bits.t] - allow multiplications wider than 62 \
                 bits"
  =
  let module A = Ark_bls12_381_g1 in
