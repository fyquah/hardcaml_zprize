...
module External = struct
  let potential_dl_filenames =
    ...
    in
    [ dir ^/ "libs/rust/ark_bls12_381_g1/target/debug/libark_bls12_381_g1." ^ extension ]
  ;;
  ...
  let () =
    ...
    let filename =
      ...
      | None ->
        failwith
          "Cannot find Rust ark_bls12_381_g1 - did you build it? Run `cargo build` in \
           rust/ark_bls12_381_g1 first."
    in
  ...
  let free = foreign "ark_bls12_381_g1_free" (ptr affine @-> returning void)

  let new_ =
    foreign
      "ark_bls12_381_g1_new"
      (ptr int64_t @-> ptr int64_t @-> bool @-> returning (ptr affine))
  ;;

  let add =
    foreign "ark_bls12_381_add" (ptr affine @-> ptr affine @-> returning (ptr affine))
  ;;

  let neg = foreign "ark_bls12_381_g1_neg" (ptr affine @-> returning (ptr affine))
  let mul = foreign "ark_bls12_381_mul" (ptr affine @-> int64_t @-> returning (ptr affine))

  let subgroup_generator =
    foreign "ark_bls12_381_g1_subgroup_generator" (void @-> returning (ptr affine))
  ;;

  let coeff_a = foreign "ark_bls12_381_g1_coeff_a" (ptr int64_t @-> returning void)
  let coeff_b = foreign "ark_bls12_381_g1_coeff_b" (ptr int64_t @-> returning void)
  let modulus = foreign "ark_bls12_381_g1_modulus" (ptr int64_t @-> returning void)

  let get_x =
    foreign "ark_bls12_381_g1_get_x" (ptr affine @-> ptr int64_t @-> returning void)
  ;;

  let get_y =
    foreign "ark_bls12_381_g1_get_y" (ptr affine @-> ptr int64_t @-> returning void)
  ;;

  let get_infinity =
    foreign "ark_bls12_381_g1_get_infinity" (ptr affine @-> returning bool)
  ;;

  let is_on_curve = foreign "ark_bls12_381_g1_is_on_curve" (ptr affine @-> returning bool)

  let equal =
    foreign "ark_bls12_381_g1_equal" (ptr affine @-> ptr affine @-> returning bool)
  ;;
end
